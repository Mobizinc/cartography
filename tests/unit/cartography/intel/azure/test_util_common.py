...
def test_copy_properties_lifts_the_wire_spelling_to_the_key_the_models_read():
    # Arrange
    disk = {"properties": {"diskSizeGB": 128}}

    # Act
    copy_properties(disk, {"disk_size_gb": ("disk_size_gb", "diskSizeGB")})
...
def test_copy_properties_never_overwrites_a_key_that_is_already_there():
    # Arrange
    disk = {"disk_size_gb": 64, "properties": {"diskSizeGB": 128}}

    # Act
    copy_properties(disk, {"disk_size_gb": ("disk_size_gb", "diskSizeGB")})

    # Assert
    assert disk["disk_size_gb"] == 64
...
def test_copy_properties_lifts_a_nested_key_with_no_properties_block():
    ...
    and camelCase."""
    # Act
    lifted = copy_properties({"vmSize": "Standard_D2s_v3"}, {"vm_size": ("vmSize",)})

    # Assert
    assert lifted == {"vmSize": "Standard_D2s_v3", "vm_size": "Standard_D2s_v3"}


def test_copy_properties_leaves_an_absent_field_absent():
    # Act
    lifted = copy_properties({"properties": {}}, {"os_type": ("osType",)})

    # Assert
    assert lifted == {"properties": {}}
